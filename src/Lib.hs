...
consumeAndProduce = do
  ...
  forM_ lattices $ \(p1, l1) -> do
    ...
    traceM . show $ x
    case x of
      Just (p2, Consumer l2) -> when ((p1, l1) == (p2, l2)) (trace "removing piece" $ removePiece p1)
      _ -> return ()
...
buildBoard = Board {
  ...
  _transitions = mempty,
  _ground = M.fromList
    [ ((0, 1), (Placement (0, 1) (hexRotation 5), Consumer (packLattice
      [(Fire, (0, 0), [(1, 0)])
      --,(Fire, (1, 0), [])
      ])))
    , ((1, 0), (Placement (1, 0) (hexRotation 0), Producer (packLattice
      [(Fire, (0, 0), [(1, 0)])
      --,(Fire, (1, 0), [])
      ])))
    ],
